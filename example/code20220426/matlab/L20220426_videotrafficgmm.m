...
% is also a first step prior to performing more sophisticated tasks such as tracking 
% or categorization of vehicles by their type.
% 
% This example shows how to use the foreground detector and blob analysis 
% to detect and count cars in a video sequence. It assumes that the camera is 
% stationary. The example focuses on detecting objects. To learn more about tracking 
% objects, see the example titled <matlab:showdemo('MotionBasedMultiObjectTrackingExample') 
...
% obtaining an initial video frame in which the moving objects are segmented from 
% the background. This helps to gradually introduce the steps used to process 
% the video.
% 
...
%% 
% Next, we find bounding boxes of each connected component corresponding 
...
% the video frame. We display the number of found cars in the upper left corner 
